...
- Alignment markers carry meaning: `--:` right-aligns numbers (`Min`).
...
- One table, chronological (`Day` → `Cuisine`). Columns: `Day` · `Dish` · `Sides` · `Cuisine`.

## Maintaining alignment
When adding/removing rows, re-pad the affected column to the new longest cell. If a row's `Dish` or `Cuisine` is longer than the current column, widen the whole column (header, separator, and every row) to match.
